...
impl<'a> PropsLoader<'a> {
    ...
    pub fn load<G: Gpu>(&mut self, world: &mut GameWorld, renderer: &mut Renderer, gpu: &mut G, slice: &Json, mask: u32) -> usize {
        ...
        for prop in props {
            ...
            let random_yaw = entry.get("randomYaw").and_then(Json::as_bool).unwrap_or(false);
            ...
            if let Some(glb_ref) = entry.get("glb").and_then(Json::as_str) {
                ...
                let tint = ph.get("tint").and_then(Json::as_str).map(parse_hex).unwrap_or([0.43, 0.4, 0.34, 1.0]);
                let (yaw, _) = placement(rotation, random_yaw, id, sw, sh, 1.0, 1.0);
                let mesh = placeholder_cube(renderer, gpu);
                let material = renderer.add_material(tint);
                let e = world.spawn();
                world.set_component(
                    e,
                    Transform {
    ...
    fn ensure_model<G: Gpu>(&mut self, renderer: &mut Renderer, gpu: &mut G, glb_ref: &str) -> Option<()> {
        ...
        let local = format!("{}{}", self.assets_dir, public.strip_prefix("/assets").unwrap_or(&public));
...
fn placement(rotation: f32, random_yaw: bool, id: &str, sw: f32, sh: f32, fx: f32, fz: f32) -> (f32, f32) {
    ...
    let fit_w = if use_random { target_w.min(target_d) } else { target_w };
    let fit_d = if use_random { target_w.min(target_d) } else { target_d };
...
fn upload_model<G: Gpu>(renderer: &mut Renderer, gpu: &mut G, doc: &GlbDocument) -> Option<PropModel> {
    ...
    for (ni, node) in doc.nodes.iter().enumerate() {
        ...
        for prim in &mesh.primitives {
            ...
            for i in 0..prim.positions.len() {
                ...
                let w = g.transform_point(vec3(p[0], p[1], p[2])).add(offset);
                ...
                verts.extend_from_slice(&[w.x, w.y, w.z, wn.x, wn.y, wn.z, uv[0], uv[1]]);
            }
...
mod tests {
    ...
    fn hash_yaw_deterministic_and_in_range() {
        ...
        assert_eq!(a, b);
        assert!(a >= 0.0 && a < core::f32::consts::PI * 2.0);
...
impl WorldScene {
    ...
    pub fn build<G: Gpu>(
        ...
        world.set_component(
            sun,
            DirectionalLight { dir: vec3(-0.4, -1.0, -0.3).normalize(), color: [1.0, 0.98, 0.92], cast_shadows: true },
        );
        ...
        world.set_component(
            ...
            Camera {
                viewport_id: 0,
                order: 0,
                projection: Projection::Perspective { fovy: 45.0_f32.to_radians(), near: 0.5, far: 2000.0 },
                target: CamTarget::Screen(RectNorm::FULL),
                clear: ClearSpec { color: Some([0.788, 0.678, 0.510, 1.0]), depth: Some(1.0) },
        ...
        Ok(WorldScene { world, renderer, camera, center, orbit })
    }

    pub fn animate(&mut self, frame: u64) {
        use successor_engine_render::components::Camera;
        let angle = frame as f32 * 0.01;
        let eye = self.center.add(vec3(angle.cos() * self.orbit, self.orbit * 0.8, angle.sin() * self.orbit));
