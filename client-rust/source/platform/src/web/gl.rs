...
pub fn shader_source(shader: u32, src: &[u8]) {
    unsafe { glShaderSource(shader, src.as_ptr(), src.len() as u32); }
}
...
pub fn get_shader_info_log(shader: u32, buf: &mut [u8]) -> usize {
    unsafe {
        glGetShaderInfoLog(shader, buf.as_mut_ptr(), buf.len() as u32) as usize
    }
}
...
pub fn get_program_info_log(program: u32, buf: &mut [u8]) -> usize {
    unsafe {
        glGetProgramInfoLog(program, buf.as_mut_ptr(), buf.len() as u32) as usize
    }
}
...
pub fn delete_texture(texture: u32) {
    unsafe { glDeleteTexture(texture); }
}
...
pub fn tex_image_2d(
    ...
    unsafe { glTexImage2D(target, level, internal_format, width, height, border, format, type_, ptr, len); }
}
...
pub fn buffer_data(target: u32, data: &[u8], usage: u32) {
    unsafe { glBufferData(target, data.as_ptr(), data.len() as u32, usage); }
}
...
pub fn delete_vertex_array(vao: u32) {
    unsafe { glDeleteVertexArray(vao); }
}
...
pub fn draw_elements(mode: u32, count: i32, type_: u32, offset: u32) {
    unsafe { glDrawElements(mode, count, type_, offset); }
}
...
pub fn delete_framebuffer(fbo: u32) {
    unsafe { glDeleteFramebuffer(fbo); }
}
...
pub fn draw_buffers(buffers: &[u32]) {
    unsafe { glDrawBuffers(buffers.as_ptr(), buffers.len() as u32); }
}
