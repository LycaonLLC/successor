...
pub const CIRCLE_COLLISION_RADIUS_MILLI: i32 = 300;
pub const CIRCLE_TRACE_SKIN_MILLI: i32 = 2;
...
pub fn resolve_circle_move_milli(
    origin: CirclePoint,
    delta_x: i32,
    delta_y: i32,
    radius_milli: i32,
...
fn trace_vertical_face(
    ...
    let top = i128::from(span.0) * i128::from(den);
    let bottom = i128::from(span.1) * i128::from(den);
...
fn trace_horizontal_face(
    ...
    let left = i128::from(span.0) * i128::from(den);
    let right = i128::from(span.1) * i128::from(den);
...
mod tests {
    ...
    #[test]
    fn adjacent_aabb_seam_does_not_snag_tangential_motion() {
        let blockers = [
    ...
    #[test]
    fn inside_corner_manifold_stops_without_tunneling() {
        let blockers = [
